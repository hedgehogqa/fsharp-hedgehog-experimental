...
let ``Should generate Or with same type for both parameters``() =
    property {
        ...
        let! result = GenX.autoWith<Or<int, int>> config
...
let ``Should generate And with same type for both parameters``() =
    property {
        ...
        let! result = GenX.autoWith<And<int>> config
...
let ``Should generate OutOfOrder with parameters in different order``() =
    property {
        let! i, s, f = GenX.auto<int * string * float>
        let config =
            GenX.defaults
            |> AutoGenConfig.addGenerator (Gen.constant i)
        ...
        let! result = GenX.autoWith<OutOfOrder<int, string, float>> config
